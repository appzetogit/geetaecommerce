                ...
                {paginatedOrders.length === 0 ? (
                  ...
                  paginatedOrders.map((order, index) => (
                    <motion.tr key={order.id} className="hover:bg-teal-50/30 transition-colors group" variants={itemVariants} custom={index}>
                      ...
                      <td className="px-6 py-4"><span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(order.status)}`}><span className="w-1.5 h-1.5 rounded-full mr-1.5 bg-green-500"></span>{order.status}</span></td>
