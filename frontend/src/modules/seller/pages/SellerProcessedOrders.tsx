          ...
          <div className="flex flex-col xl:flex-row gap-4 items-end xl:items-center justify-between">
            ...
            <div className="flex flex-col md:flex-row gap-4 w-full xl:w-auto">
              ...
              <div className="w-full md:w-auto self-end">
                <button onClick={handleExport} className="w-full md:w-auto flex items-center justify-center gap-2 bg-seller-600 hover:bg-seller-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-all shadow-sm hover:shadow active:scale-95"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4M7 10l5 5 5-5M12 15V3" strokeLinecap="round" strokeLinejoin="round"/></svg><span>Export</span></button>
              </div>
            </div>
          </div>
        </div>
                ...
                {paginatedOrders.length === 0 ? (
                  ...
                  paginatedOrders.map((order, index) => (
                    <motion.tr key={order.id} className="hover:bg-seller-50/30 transition-colors group" variants={itemVariants} custom={index}>
                      ...
                      <td className="px-6 py-4"><span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(order.status)}`}><span className="w-1.5 h-1.5 rounded-full mr-1.5 bg-blue-500"></span>{order.status}</span></td>
