...
export default function SellerReplaceRequests() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'Approved': return 'bg-seller-100 text-seller-800 border border-seller-200';
  ...
  return (
    <motion.div className="space-y-6" initial="hidden" animate="visible" variants={containerVariants}>
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white p-4 rounded-xl shadow-sm border border-neutral-200">
        ...
        <div className="flex items-center gap-2 text-sm bg-neutral-50 px-3 py-1.5 rounded-lg border border-neutral-200"><Link to="/seller" className="text-seller-600 hover:text-seller-700 font-medium">Home</Link><span className="text-neutral-400">/</span><span className="text-neutral-600">Replace Requests</span></div>
      </div>
      <div className="bg-white rounded-xl shadow-sm border border-neutral-200 overflow-hidden">
        <div className="bg-seller-600 text-white px-4 sm:px-6 py-2 sm:py-3"><h2 className="text-base sm:text-lg font-semibold">Replace Requests List</h2></div>
            ...
            <div className="flex flex-col md:flex-row gap-4 w-full xl:w-auto">
              <div className="w-full md:w-64">
                ...
                <div className="relative">
                  <input type="text" value={searchQuery} onChange={(e) => { setSearchQuery(e.target.value); setCurrentPage(1); }} className="w-full pl-10 pr-4 py-2 bg-white border border-neutral-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-seller-500/20 focus:border-seller-500 transition-all placeholder:text-neutral-400" placeholder="Search order ID or customer..." />
        ...
        {loading && <div className="flex flex-col items-center justify-center p-12"><div className="animate-spin rounded-full h-10 w-10 border-b-2 border-seller-600 mb-4"></div><div className="text-neutral-500 font-medium">Loading requests...</div></div>}
        {error && !loading && <div className="p-6 text-center"><div className="text-red-600 mb-2">{error}</div><button onClick={() => window.location.reload()} className="text-seller-600 font-medium hover:underline">Try Again</button></div>}
        {!loading && !error && (
              ...
              <tbody className="divide-y divide-neutral-100 bg-white">
                {requests.length === 0 ? (
                  ...
                  requests.map((request, index) => (
                    <motion.tr key={request.id} className="hover:bg-seller-50/30 transition-colors group" variants={itemVariants} custom={index}>
                      <td className="px-6 py-4 text-sm font-medium text-neutral-900"><span className="font-mono text-seller-700 bg-seller-50 px-2 py-0.5 rounded border border-seller-100">#{request.orderId}</span></td>
