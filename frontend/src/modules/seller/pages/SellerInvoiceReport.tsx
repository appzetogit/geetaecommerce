...
import { getSellerPOSOrders, updateOrderStatus, ReportOrder } from "../../../services/api/orderService";
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 py-4">
          ...
          {showCustomDatePicker && (
            <div className="mt-4 p-4 bg-seller-50/30 rounded-2xl border border-seller-100 flex gap-4 animate-slide-left">
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-black text-seller-400 uppercase ml-1 tracking-widest">START DATE</label>
                <input type="date" value={customDateRange.start} onChange={(e) => setCustomDateRange({ ...customDateRange, start: e.target.value })} className="px-3 py-1.5 bg-white rounded-lg border border-seller-200 text-sm font-bold text-gray-700 outline-none focus:ring-2 focus:ring-seller-500 transition-all shadow-sm" />
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-black text-seller-400 uppercase ml-1 tracking-widest">END DATE</label>
                <input type="date" value={customDateRange.end} onChange={(e) => setCustomDateRange({ ...customDateRange, end: e.target.value })} className="px-3 py-1.5 bg-white rounded-lg border border-seller-200 text-sm font-bold text-gray-700 outline-none focus:ring-2 focus:ring-seller-500 transition-all shadow-sm" />
              </div>
            </div>
          )}
            ...
            <table className="w-full text-sm min-w-[1200px]">
                ...
                <tr className="bg-gray-50/50 border-b border-gray-100">
                  <th className="px-6 py-4 text-left">
                    <input
                      ...
                      className="w-4 h-4 text-seller-600 rounded border-gray-300 focus:ring-seller-500 cursor-pointer transition-all"
                    />
                  </th>
              ...
              <tbody className="divide-y divide-gray-50">
                {loading ? (
                  ...
                  orders.map((item: any) => (
                    <tr key={item._id} className="hover:bg-seller-50/30 transition-colors group">
                      ...
                      <td className="px-4 py-4 min-w-[140px]">
                        {editMode ? (
                           <select
                            ...
                            className={`w-full px-2 py-1 text-[10px] font-black rounded-md border border-gray-200 outline-none focus:border-seller-500 transition-all uppercase tracking-widest ${item.status === 'Delivered' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>
                      ...
                      <td className="px-6 py-4 text-right">
                         <Link to={`/seller/orders/${item._id}`} className="px-4 py-2 bg-white border border-gray-100 text-seller-600 rounded-xl text-[10px] font-black hover:bg-seller-600 hover:text-white hover:border-seller-600 transition-all active:scale-95 shadow-sm">Details</Link>
                      </td>
                    </tr>
                  ))
                )}
        ...
        {pagination.pages > 1 && (
          <div className="mt-6 flex flex-wrap items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
            <div className="text-xs font-black text-gray-400 uppercase tracking-widest">
              Showing <span className="text-seller-600">{orders.length}</span> of <span className="text-seller-600">{pagination.total}</span> Invoices
            </div>
