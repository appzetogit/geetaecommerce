        ...
        <form id="admin-product-form" onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-neutral-200">
            ...
            <div className="p-6 space-y-6 border-x border-b border-neutral-200 rounded-b-xl">
              {/*
                Main Product (parent) Barcode callout — admin side.

                Mirrors the same callout added to SellerAddProduct.tsx. The
                parent barcode field already exists further down the form
                ("Item Code & Multiple Barcodes" / "Barcodes (EAN/UPC)"), but
                admins were missing it once the Variations section was
                expanded because all attention shifts to the per-row Barcode
                column. With `formData.barcode` left empty, POS scanning of
                the parent label falls through to the variant match in
                `onScanSuccess` (AdminPOSOrders.tsx lines ~891-917), giving
                the customer-reported "scan adds a variant instead of the
                main product" behaviour.

                Pure UI addition — same `formData.barcode` field, same
                `addBarcode('product', ...)` / `removeBarcode('product', ...)`
                handlers. No backend or save-path change.
              */}
